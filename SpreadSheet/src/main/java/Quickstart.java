...
public class Quickstart {
    ...
    public static void main(String[] args) throws Exception {
    	...
    	QuickStartPOJO qs=new QuickStartPOJO();
        ...
        String range = "MyTestSheet!A1:D3";
        ValueRange response = service.spreadsheets().values().get(spreadsheetId, range).execute();
        List<List<Object>> rowValues = response.getValues();
        
        if (rowValues != null && rowValues.size() > 0) {
        	for (List row : rowValues) 
        	{
        		for (int i = 0; i < row.size(); i++) {
					System.out.print( row.get(i) + " " );
					
					qs.setName((String)row.get(0));
					qs.setLname((String)row.get(1));
					qs.setUSN((String)row.get(2));
